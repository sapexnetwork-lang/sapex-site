/*
 * blog-enhance.js (renamed from blog-paywall.js)
 * Included on every blog/<slug>.html page (see Social_bot.py's
 * _render_post_page). Renamed because ad-blocker/privacy-extension
 * filter lists (uBlock Origin, EasyList/EasyPrivacy, Brave Shields,
 * etc.) commonly block any resource with "paywall" literally in its
 * filename — silently, at the network level, before the script ever
 * runs. That produces no JS error at all, which is exactly the
 * "nothing happens, no warnings either" symptom this was hitting.
 *
 * Two jobs:
 *
...
(function () {
    ...
    async function loadAuthAndPlan() {
        ...
        try {
            ...
            console.warn('blog-enhance.js: auth/plan check failed, defaulting to signed-out.', e);
